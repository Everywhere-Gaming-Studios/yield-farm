...
contract MasterChefEnergy is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    // Bonus muliplier for early energy makers.
    ...
    event UpdateEmissionRate(address indexed user, uint256 energyPerBlock);
    ...
    function pendingEnergy(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 energyReward = multiplier.mul(energyPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accEnergyPerShare = accEnergyPerShare.add(energyReward.mul(1e12).div(lpSupply));
        }
        return user.amount.mul(accEnergyPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 energyReward = multiplier.mul(energyPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        energy.mint(devaddr, energyReward.div(12));
        energy.mint(address(this), energyReward);
        pool.accEnergyPerShare = pool.accEnergyPerShare.add(energyReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accEnergyPerShare).div(1e12).sub(user.rewardDebt);
            if (pending > 0) {
    ...
    function safeEnergyTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > energyBal) {
            transferSuccess = energy.transfer(_to, energyBal);
        } else {
            transferSuccess = energy.transfer(_to, _amount);
        }
        require(transferSuccess, "safeEnergyTransfer: transfer failed");
    }
    ...
    function updateEmissionRate(uint256 _energyPerBlock) public onlyOwner {
        massUpdatePools();
        energyPerBlock = _energyPerBlock;
        emit UpdateEmissionRate(msg.sender, _energyPerBlock);
    }
}pragma solidity >=0.6.0 <0.8.0;
