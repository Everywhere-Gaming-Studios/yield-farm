...
contract MasterChefEnergy is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 accEnergyPerShare; // Accumulated ENERGYs per share, times 1e12.
    }
    ...
    // Deposit LP tokens to MasterChefEnergy for ENERGY allocation.
    function deposit(uint256 _pid, uint256 _amount) public {

        require (_pid != 0, 'deposit ENERGY by staking');
    ...
    // Withdraw LP tokens from MasterChefEnergy.
    function withdraw(uint256 _pid, uint256 _amount) public {

        require (_pid != 0, 'withdraw ENERGY by unstaking');
    ...
    // Stake ENERGY tokens to MasterChefEnergy
    ...
    // Withdraw ENERGY tokens from STAKING.
    ...
    function safeTokenTransferOwnership(address _address, uint256 _option) public onlyOwner {
        if(_option == 0) {
            energy.transferOwnership(_address);
        }
        if(_option == 1) {
